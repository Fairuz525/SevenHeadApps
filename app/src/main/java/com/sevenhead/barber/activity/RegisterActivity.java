...
public class RegisterActivity extends AppCompatActivity {
    ...
    EditText txtName, txtUsername, txtPassword, txtemail, txtnotelp, txtalamat;
    Button btnDaftar, btnKeLogin;
    DatabaseHelper dbHelper;
    SQLiteDatabase db;
    String name, username, email, notelp, alamat, password;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        txtemail = findViewById(R.id.reg_unm);
        txtalamat = findViewById(R.id.reg_alamat);
        txtnotelp = findViewById(R.id.reg_nomer);
        ...
        btnDaftar.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                try {
                    if (username.trim().length() > 0 && password.trim().length() > 0 && name.trim().length() > 0 && email.trim().length() > 0 && alamat.trim().length() > 0 && notelp.trim().length() > 0) {
                        dbHelper.open();
                        dbHelper.Register(username, password, name, email, alamat, notelp);
